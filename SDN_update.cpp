...
//if PreNode not send yet, push into arr. After traverse the PreNode, ctrl_packet_event
//if PreNode has been visited, recurrent DetermineRout=> the child has been sand, child as parent, so its child can be sand too.
//(parents must be point upward, so if child is updated, mean its child can be updated too, with no loop)
void DetermineRout(int con_id, unsigned int id, unsigned int mat, int* _r_arr) { //only in update time, w_order=2
    ...
    if(_r_arr[id] == 2) {
        //cout << "size: " << node::id_to_node(id)->PreNode[_mat].size() << endl;
        for(auto it = node::id_to_node(id)->PreNode[_mat].begin(); it != node::id_to_node(id)->PreNode[_mat].end(); it++) {
            //cout << "it: " << *it << endl;
            //cout << "it's r_arr: " << _r_arr[*it] << endl;
            if(_r_arr[*it] != 0) {//already send
...
void SDN_controller::recv_handler (packet *p) {
    ...
    if(p->type() == "SDN_ctrl_packet" && p->getHeader()->getDstID() == getNodeID()) { //this is ack
        ...
        _r_arr[mat] = 2;//we need to let _r_arr[mat] always be 2, so in determine rout we can processing
        ...
        if(n_update[mat].empty() == 0) {
            ...
            if(n_update[mat].empty()) {
                ...
                if(r_arr[mat][mat] != 2) { //when empty, let we do the mat round
                    DetermineRout(getNodeNum()-1, mat, node::id_to_node(mat)->table[mat], _r_arr);
                    r_arr[mat][mat] = 2;
                    return;
                }   
            } 
            if(r_arr[mat][mat] == 2){ //only if r_arr[mat][mat] == 2, we do this process, so the first round update chld process will be discard
...
int main()
{
    ...
    //ctrl_flag: 用來判斷是第一次還是第二次，已做過ctrl_packet的話會更新ctrl_flag。ex: 如果是在InsTime跟UdpTime區間間，只會進行一次routing update
    int w_order = 0;//weight order
    vector<pair<int ,int> > temp_table[10000];//用來記錄第一次的table，如果第二次更新有相同的msg則略過
    map<int, int> temp_pre_num[10000];
    static int* record_arr = new int[10000]();
    //for(int i = 0; i<1000; i++)
        //arr[i] = new int[1000]{0};
    
    /*0->initial state
      1->send ctrl packet to switch
      2->ack, return from switch to controller

      if all the rout of this round is over, all the packet has mark with ack, then go next round.
    */
    while(1) {
        ...
        if(w_order == 1 && ctrl_flag[0] == 1 && ctrl_flag[1] == 0) {
            for(int i = 0; i < dests; i++) {
                for(int id = 0; id < nodes; id++) {
                    node::id_to_node(id)->Pre_num[dest[i]] = 0;
                    node::id_to_node(id)->PreNode[dest[i]].clear();
                    //r_arr[dest[i]][id] = 0;
                    //cout << "check whether is clear: " << node::id_to_node(id)->PreNode[dest[i]].empty() << endl;
                }
            }
        }
        //cout << endl << "w_order: " << w_order << endl;
        if(ctrl_flag[w_order] == 0 && w_order != (-1)) { //this table(new/old) is not build yet, then build it!
            for(unsigned int i = 0; i < dests; i++) {
                Build_Table(dest[i], nodes, w_order);
                for(unsigned int k = 0; k < node::getNodeNum(); k++)
                    node::id_to_node(k)->initial_flag();
            } 
        }
        
        if(t < ctrl_t) { //send data packet first, but there is no rout to follow so can't send successfully
            ...
            if(!ctrl_flag[w_order]) {
                //if not send ctrl packet yet, then send!
                /*
                in here, the first time update order of ctrl_packet_event is input here.
                */
               if(w_order == 0) {
                   for(unsigned int id = 0; id < nodes; id++) { //id
                        // in second update, id need to be handling in right order. => using DetermineRout()
                        //string* msg = new string [dests];
                        int* mat = new int[dests];
                        int* act = new int[dests]; 
                        int _flag = 0;
                        for(unsigned int i = 0; i < dests; i++) {
                            if(id == dest[i]) {//id等於dst時略過
                                mat[i] = -1;
                                act[i] = -1;
                                continue;
                            }
                        ...
                        _flag = 0;
                        int c = 0;
                        for(unsigned int i = 0; i < dests; i++, c++) {
                            if(mat[i] == -1 || act[i] == -1) {//id = dst
                                //cout << "temp_pre_num regist: id: " << id << " " << node::id_to_node(id)->Pre_num[dest[i]] << endl;
                            ...
                            else if(w_order == 1) {//與第一次的table做比較，有相同msg則結束temp_table的比對(w_order = 1)
                                for(auto it = temp_table[id].begin(); it != temp_table[id].end(); it++) {
                                    if(*it == make_pair(mat[i], act[i])) {
                                        _flag = 1;
                                        break;
                                    }
                                }
                   ...
                   /*
                    cout << "check pre node: " << node::id_to_node(4)->PreNode[4][0] << endl;
                    cout << "check pre node: " << node::id_to_node(4)->PreNode[4][1] << endl;
                    cout << "size: " << node::id_to_node(4)->PreNode[4].size() << endl;
                    for(auto it = node::id_to_node(4)->PreNode[4].begin(); it != node::id_to_node(4)->PreNode[4].end(); it++) 
                        cout << *it << endl;
                    */
